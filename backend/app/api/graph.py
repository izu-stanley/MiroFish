...
def build_graph():
    """
    API 2: Build graph by project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is already building. Do not resubmit. Add force: true to force rebuild",
        ...
        def build_task():
            ...
            try:
                build_logger.info(f"[{task_id}] Starting graph build... (local)")
                ...
                # Wait for processing (local mode has no wait)
                task_manager.update_task(
                    task_id,
...
def get_graph_data(graph_id: str):
    ...
    """
        try:
            from ..services.local_graph_storage import get_graph_data as local_get
            graph_data = local_get(graph_id)
...
def delete_graph(graph_id: str):
    ...
    """
        try:
            from ..services.local_graph_storage import delete_graph as local_delete
            local_delete(graph_id)
