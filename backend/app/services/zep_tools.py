...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### Key facts (cite these in your report)")
...
class ZepToolsService:
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a question analysis expert. Break a complex question into sub-questions that can be independently observed in the simulation.

Requirements:
1. Each sub-question should be specific enough to find relevant agent behavior or events
...
5. Generate sub-questions in English"""
...
Return JSON with sub_queries list in English."""
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "Requirements:\n"
            "1. Answer in natural language only; do not call any tools\n"
            "2. Do not return JSON or tool-call format\n"
            "3. Do not use Markdown headers (#, ##, ###)\n"
            "4. Answer each question by number, starting each with 'Question X:' (X = question number)\n"
        ...
        try:
            ...
            if not api_result.get("success", False):
                ...
                result.summary = f"Interview API call failed: {error_msg}. Check OASIS simulation environment."
                return result
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation may be stopped. Ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planner. Select the best agents to interview based on the requirement.

Selection criteria:
1. Agent's role/profession is relevant to the topic
2. Agent may hold unique or valuable views
3. Diversify perspectives (supporters, opponents, neutrals, experts)
4. Prefer agents directly involved in the event
...
Select up to {max_agents} agents to interview and explain your reasoning."""
...
7. Generate questions in English
...
Generate 3-5 interview questions in English."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Generate an interview summary based on the interviewees' responses.

Summary requirements:
1. Extract main points from each party
2. Note consensus and disagreement
...
- Do not use dividers (---, ***)
- Use standard quotation marks for direct quotes
