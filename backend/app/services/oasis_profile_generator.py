...
将本地图谱中的实体转换为OASIS模拟平台所需的Agent Profile格式

使用 Cursor Agent 生成详细人设，无 Zep Cloud 依赖。
"""
...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        本地模式：使用实体已有的 related_edges/related_nodes，无 Zep 检索。
        """
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 4. 本地模式无 Zep 检索，使用实体已有信息
        zep_results = self._search_zep_for_entity(entity)
        if zep_results.get("facts"):
            new_facts = [f for f in zep_results["facts"] if f not in existing_facts]
            if new_facts:
                context_parts.append("### 额外事实信息\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media persona generation expert. Generate detailed, realistic personas for opinion simulation, maximizing fidelity to the given context. Return valid JSON only; all string values must not contain unescaped newlines. Use English for all output."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximizing fidelity to the given context.
...
2. persona: Detailed persona description (~2000 chars plain text), including:
   ...
   - Personality (MBTI, core traits, emotional expression style)
   - Social media behavior (posting frequency, content preferences, interaction style, language)
   - Stance (attitude toward the topic, what may provoke or move them)
   - Distinctive traits (catchphrases, special experiences, hobbies)
   - Personal memory (how this individual relates to the event and their prior actions/reactions)
3. age: Integer age
...
- persona must be one coherent text block
- Use English for all fields
- Content must align with entity information
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for this organization/group entity, maximizing fidelity to the given context.
...
2. persona: Detailed account persona (~2000 chars plain text), including:
   ...
   - Communication style (language, common expressions, taboo topics)
   - Content characteristics (content types, posting frequency, active hours)
   - Stance (official position on core topics, handling of controversy)
   - Special notes (represented demographics, operating habits)
   - Institutional memory (how this org relates to the event and prior actions/reactions)
3. age: Fixed 30 (virtual age for org accounts)
4. gender: Fixed "other" (org accounts use "other" for non-individual)
5. mbti: MBTI type for account style (e.g. ISTJ = formal/conservative)
6. country: Country name in English (e.g. "United States")
7. profession: Organization function description
8. interested_topics: Array of focus areas

Important:
- All field values must be strings or numbers, no null
- persona must be one coherent text block, no newlines
- Use English for all fields
- age must be 30, gender must be "other"
- Org account voice must match its identity"""
